...
function arrayToTree(
  ...
  array.forEach((item) => {
    ...
    if (
      Array.isArray(item[options.children]) &&
      item[options.children].length
    ) {
      // 去重合并数组
...
function splicParentsUntil(
  ...
  let coordinate_item = data.find(
    (i) => i[options.pathId] === coordinate[options.pathId]
  );
  if (!coordinate_item) return "";
  if (!coordinate_item[options.pathParents])
    return coordinate_item[options.pathName];
